...
# Fetch semua data area
def fetch_all_areas(token):
    all_data = []
    offset = 0
    limit = PAGE_CHUNK

    while True:
        res = get_all_areas(token, offset=offset, limit=limit)
        if not res:
            break
        if res.status_code != 200:
            try:
                err = res.json().get("error") or res.json().get("message") or "Gagal memuat data area."
            except Exception:
                err = "Gagal memuat data area."
            st.error(err)
            break
...
def render_grid(df):
    ...
    gb.configure_column("id_area", header_name="ID AREA", width=150, editable=False)
...
def app():
    ...
    if st.button("⬆️ Upload Area"):
        st.session_state.page = "upload_area"
        st.rerun()
        return

    # Refresh data hanya jika belum ada atau flag refresh aktif
    if "area_data" not in st.session_state or st.session_state.get("refresh_area", True):
        with st.spinner("Memuat semua data area..."):
            all_data, total_count = fetch_all_areas(token)
            st.session_state["area_data"] = all_data
            st.session_state["area_total"] = total_count
            st.session_state["refresh_area"] = False

    data = st.session_state.get("area_data", [])
    total_rows = st.session_state.get("area_total", len(data))

    if not data:
        st.info("Tidak ada data area yang ditemukan.")
        return

    st.markdown(f"**Total Data Area: {total_rows}**")
    ...
    if st.button("💾 Simpan Perubahan"):
        ...
        original_dict = {row["id_area"]: row for row in data}

        for _, row in updated_df.iterrows():
            id_area = row["id_area"]
            if id_area not in original_dict:
                continue

            original_row = original_dict[id_area]

            if row["description"] != original_row.get("description"):
                res = update_area(token, id_area, row["description"], updateby)
    ...
    if st.button("🗑️ Hapus Data Terpilih"):
        if selected_rows.empty:
            ...
            ids_to_delete = selected_rows["id_area"].tolist()
            res = delete_areas(token, ids_to_delete)
            if res and res.status_code == 200:
    ...
    with col2:
        st.markdown(
            f"### Menampilkan {len(st.session_state['area_data'])} / {total_rows} baris",
