...
from utils.api.product.product_dist_api import insert_product_dist

def generate_template():
    df = pd.DataFrame(columns=["pcode_dist", "pcodename", "branch_dist"])
    buffer = BytesIO()
...
def process_upload(file, username):
    try:
        df = pd.read_excel(file, dtype={"pcode_dist": str, "branch_dist": str})
    except Exception:
        st.error("❌ File tidak valid. Pastikan file Excel benar.")
        return None
    
    # VALIDASI KOLOM
    required_cols = ["pcode_dist", "pcodename", "branch_dist"]
...
def app():
    ...
    st.title("⬆️ Upload Product Dist")

    # DOWNLOAD TEMPLATE
    st.subheader("📄 Download Template Product Dist")
    template_file = generate_template()
    st.download_button(
        label="Download Template",
        data=template_file,
        file_name="template_product_dist.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    )

    # BELUM UPLOAD
    if not st.session_state.upload_done:
        st.subheader("📤 Upload Data Product DIST")
    ...
    # JIKA SUDAH SELESAI
        ...
        # BUTTON BACK
        st.markdown("---")
        if st.button("⬅️ Kembali ke Data Product DIST"):
            st.cache_data.clear()
            st.session_state["refresh_product_dist"] = True
            st.session_state.page = "product_dist"
            st.session_state.upload_done = False
            st.session_state.upload_result = None
            st.rerun()
