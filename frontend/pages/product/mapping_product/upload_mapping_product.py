...
from utils.api.product.mapping_product_api import insert_mapping_product

# GENERATE TEMPLATE XLSX
def generate_template():
    df = pd.DataFrame(columns=["branch_dist", "pcode_dist", "pcode_prc"])
    buffer = BytesIO()
...
def process_upload(file, username):
    ...
    required_cols = ["branch_dist", "pcode_dist", "pcode_prc"]
...
def app():
    ...
    st.title("⬆️ Upload Mapping Product")

    # Download template
    st.subheader("📄 Download Template Mapping Product")
    st.download_button(
        label="Download Template",
        data=generate_template(),
        file_name="template_mapping_product.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    )

    # UPLOAD
    if not st.session_state.upload_done:
        st.subheader("📤 Upload Data Mapping Product")
    ...
    # HASIL UPLOAD
        ...
        # KEMBALI KE HALAMAN Mapping Product
        if st.button("⬅️ Kembali ke Data Mapping Product"):
